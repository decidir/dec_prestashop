...
class Payway extends PaymentModule
{
    ...
    {
        $this->name = 'payway';
        $this->displayName = $this->l('payway');
        $this->description = $this->l('payway - Payment Gateway');
        $this->paymentMethodName = $this->l('Credit Card (payway)');
    ...
    public function createOrderStates()
    {
        ...
        if (!in_array('review', $sts)) {
            $this->createOrderState('review', 'payway - Review', true);
        }
        if (!in_array('approved', $sts)) {
            $this->createOrderState('approved', 'payway - Approved', true);
        }
        if (!in_array('rejected', $sts)) {
            $this->createOrderState('rejected', 'payway - Rejected', true);
        }
        if (!in_array('annulled', $sts)) {
            $this->createOrderState('annulled', 'payway - Annulled', true);
        }
    }
    ...
    {
        $dbx = _DB_PREFIX_;
        $sql = "SELECT * FROM {$dbx}payway_banks $whr";
        $res = Db::getInstance()->executeS($sql);
        $res = Tools::jsonEncode($res);
        $res = Tools::jsonDecode($res);
        return $res;
    ...
    {
        $dbx = _DB_PREFIX_;
        $sql = "SELECT * FROM {$dbx}payway_cards $whr";
        $res = Db::getInstance()->executeS($sql);
        $res = Tools::jsonEncode($res);
        $res = Tools::jsonDecode($res);
        return $res;
    ...
    public function getPromotions($only_active = false)
    {
        $prs = array();
        $dbx = _DB_PREFIX_;
        $sql = "SELECT * FROM {$dbx}payway_promotions";
        if ($only_active) {
            $sql .= " WHERE active = 1";
        }
    ...
    {
        $id = (int) $id;
        Db::getInstance()->delete('payway_installments', "id_promotion = $id");
        Db::getInstance()->delete('payway_promotions', "id_promotion = $id");
        return $id;
    }
    ...
    public function getPromotionInstallments($arrange = false, $pro = 0)
    {
        $dbx = _DB_PREFIX_;
        $sql = "
        SELECT * FROM {$dbx}payway_installments";
        $pro && $sql .= "
